...
export async function getUsageForProvider(
  ...
  switch (provider) {
    ...
    default: {
      const { isOpenAICompatibleProvider: isOAI, isAnthropicCompatibleProvider: isACI } = await import("@/shared/constants/providers");
      if (isOAI(provider) || isACI(provider)) {
        return await getOpenAICompatibleUsage(id || "", connection as never);
      }
