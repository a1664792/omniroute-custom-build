...
import { USAGE_SUPPORTED_PROVIDERS, isOpenAICompatibleProvider, isAnthropicCompatibleProvider } from "@/shared/constants/providers";
...
export function isSupportedUsageConnection(connection: ProviderConnectionLike | null): boolean {
  ...
  const isSupported =
    USAGE_SUPPORTED_PROVIDERS.includes(provider) || isCompatible;
  ...
  if (isCompatible) {
    return connection.authType === "apikey" || connection.authType === "api_key";
  }
  return (
    (connection.authType === "apikey" || connection.authType === "api_key") &&
    PROVIDER_LIMITS_APIKEY_PROVIDERS.has(provider)
  );
}
